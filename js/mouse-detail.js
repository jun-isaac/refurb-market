...
function showDestail(obj) {
	...
	obj.forEach(function (product, i) {
		// 해당 id 제품 출력
		var connect = product.connect;
		var dpi = product.dpi;
		var softwear = product.softwear;
		var rgb = product.rgb;
		var weight = product.weight;
		var cable = product.cable;
		var as = product.as;
		// console.log('name = ', name);
		console.log("image", imgUrl1);

		// product-detail.html에 각 변수값 주입
		$('.c-glay .spec1').text(connect);
		$('.c-white .spec2').text(dpi);
		$('.c-glay .spec3').text(softwear);
		$('.c-white .spec4').text(rgb);
		$('.c-glay .spec5').text(weight);
		$('.c-white .spec6').text(cable);
