...
function getData() {
	...
	var DataURL = 'https://raw.githubusercontent.com/jun-isaac/refurb-market/main/json/monitor-detail.json'
	fetch(DataURL)
		.then(function (res) {
...
function showDestail(obj) {
	...
	obj.forEach(function (product, i) {
		...
		var screen = product.screen;
		var panel = product.panel;
		var display = product.display;
		var fps = product.fps;
		var bluelight = product.bluelight;
		var as = product.as;
		// console.log('name = ', name);
		console.log("image", imgUrl1);

		// product-detail.html에 각 변수값 주입
		$('.c-glay .spec1').text(size);
		$('.c-white .spec2').text(screen);
		$('.c-glay .spec3').text(panel);
		$('.c-white .spec4').text(display);
		$('.c-glay .spec5').text(fps);
		$('.c-white .spec6').text(bluelight);
